  ...
  methods:{
    ...
    initWebsocket(){
      ...
      this.ws.onmessage = (event) =>{
        ...
        console.log('Received a message from the backend：', msgObj);
        // 如果消息类型是弹幕，则调用播放器发送弹幕
        if(msgObj.txt){
          console.log('Received a message：',msgObj);
...
<!--          数据统计-->
          <div class="danmu-bar-video-info">
            {{onWatching}}peoplare are watching, {{danmuCount}} uploaded Danmu
          </div>
<!--          弹幕操作-->
          <div class="danmu-bar-operation">
            <div class="danmu-bar-sender">
              <input class="danmu-bar-sender-input" v-model="danmuText"
                     placeholder="Sending warm wishes to witness the present moment." autocomplete="off"
                      @keyup.enter="sendDanmu">
              <el-button type="primary" @click="sendDanmu">Send</el-button>
            </div>
          </div>
...
<!--        视频投稿的up主信息-->
        ...
        <div class="danmu-box">
          <el-collapse v-model="activeNames">
            <el-collapse-item title="Danmu List" name="1">
              <el-table
                  :data="tableData"
                  style="width: 100%"
                  height="500px">
